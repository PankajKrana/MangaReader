...
struct SplashView: View {
                    ...
                    HStack(spacing: 8) {
                        ForEach(0..<3, id: \.self) { index in
                            Circle()
                                ...
                                .opacity(opacity)
                                .animation(
                                    reduceMotion ? .default : .easeInOut(duration: 0.8)
    ...
    private func startAnimations() {
        withAnimation(reduceMotion ? .none : .easeIn(duration: 1.0)) {
            backgroundOpacity = 1.0
        }

        withAnimation(reduceMotion ? .none : .spring(response: 1.0, dampingFraction: 0.6, blendDuration: 0)) {
            scaleEffect = 1.0
            opacity = 1.0
        }

        withAnimation(reduceMotion ? .none : .easeOut(duration: 0.8).delay(0.3)) {
            titleOffset = 0
        }
